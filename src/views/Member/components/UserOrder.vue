...
const getOrderList = async () => {
  try {
    ...
    // 注意：res已经是拦截器处理后的res.data
    console.log('API响应数据:', res) // 查看真实数据结构
    
    // 根据实际API结构调整（以下是常见结构示例）
    orderList.value = res?.items || res?.result?.items || []
    total.value = res?.counts || res?.result?.counts || 0
    
  } catch (error) {
    console.error('完整的请求错误:', error)
    // 可以在这里添加更友好的错误提示
