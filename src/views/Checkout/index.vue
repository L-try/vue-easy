...
const getCheckInfo = async () => {
  ...
  checkInfo.value = {
    ...res.result,
    goods: res.result.goods.filter(item => 
      cartStore.cartList.some(cartItem => 
        cartItem.skuId === item.skuId && cartItem.selected
      )
    )
  }
  console.log('支付确认')
      ...
      <div class="wrapper">
        ...
        <div class="box-body">
          <div class="total">
            ...
            <dl>
              <dt>应付总额：</dt>
              <dd class="price">{{ cartStore.selectedPrice.toFixed(2) }}</dd>
            </dl>
          </div>
        </div>
        <!-- 提交订单 -->
